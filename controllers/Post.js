...
exports.createPost = async (req, res) => {
  try {
    ...
    const userDetails = await User.findByIdAndUpdate(
      ...
      { new: true }
    ).populate("communityDetails", "userInCommunity communityName");
...
exports.deletePost = async (req, res) => {
  try {
    ...
    // Step 1: Remove post from user's array
    user.postByUser = user.postByUser.filter(id => id.toString() !== postId);
    await user.save();

    // Step 2: Remove post from community
    await Community.findByIdAndUpdate(user.communityDetails, {
      $pull: { posts: postId }
    });

    // Step 3: Delete all comments & nested replies related to the post
    ...
    // Step 4: Delete post notifications
